  ...
  module Nodes
    module Uploadable
      ...
      def upload(path, filename: nil)
        ...
        raise "Empty file - #{path}" if filesize.zero?
        ...
        self.class.each_chunk(filesize) do |start, size|
          pool.process do
            ...
            encrypted_buffer, chunk_mac = *encrypt_chunck(start, clean_buffer, rnd_node_key.aes_key,
                                                          rnd_node_key.ctr_nonce)
        ...
        _attr = serialize_attributes(n: filename)
        ...
        Nodes::Factory.build(session, resp['f'][0])
      ensure
        file&.close
      end
    end
